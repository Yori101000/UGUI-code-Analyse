...
### 11.1.4 ILayoutController — 布局控制层
...
### 11.1.5 Layout 与 RectTransform 的关系
...
## 11.4 Layout 与 RectTransform 驱动关系
...
### 11.4.3 DrivenRectTransformTracker
...
### 11.4.4 Layout 如何修改 RectTransform
...
## 11.7 ScrollRect 深度解析

### 11.7.1 基本结构

ScrollRect 继承自 `UIBehaviour`，实现拖拽、滚轮、重建、布局等多组接口，核心字段如下：
...
public virtual void OnDrag(PointerEventData eventData) {
    ...
    // Elastic 模式下对超出量做 RubberDelta 拉伸
...
### 11.7.4 LateUpdate：物理模拟中心

惯性衰减和弹性回弹都在 `LateUpdate` 中处理，不在 `OnDrag` 中。惯性使用指数衰减，并且用 `Time.unscaledDeltaTime`，不受 timeScale 影响：
...
`m_DecelerationRate` 默认 0.135：0 表示瞬间停止，1 表示永不停止。

Elastic 模式的回弹用 `Mathf.SmoothDamp` 模拟弹簧阻尼系统，永不超调，值越大回弹越慢：
...
Clamped 模式直接修正位置到边界内，没有弹性动画。

### 11.7.5 MovementType 三种模式对比
...
### 11.7.6 与 Scrollbar 联动

Content 和 Scrollbar 双向绑定：
...
### 11.7.7 ScrollRect 与 Layout 的关系
...
ScrollRect 本身也实现了 `ILayoutGroup` 和 `ILayoutController`，参与 Canvas 的 Layout 重建：`SetLayoutHorizontal` 处理 ViewRect 的 AutoHideAndExpand 逻辑并更新 Content 的 sizeDelta，`Rebuild` 在 Prelayout 阶段更新缓存、在 PostLayout 阶段更新边界。这使得 Content 尺寸变化时滚动范围能自动调整。

### 11.7.8 ContentSizeFitter + ScrollRect 的问题
...
## 11.8 大规模列表：为什么最后都要绕开 Layout

大规模列表（背包、排行榜、聊天记录）的性能瓶颈通常不在"元素数量本身"，而在 **Layout 系统的递归计算范围**——这是本章前面所有机制汇总到工程层面的直接后果：

```
新增/修改一个 item
  → 该节点 SetLayoutDirty（11.1.5）
    → 向上找到最近的 LayoutGroup 父级（11.2）
      → Layout Rebuild 递归遍历整棵 Content 树重算所有子节点（11.1.6）
        → ContentSizeFitter 反向调整 Content 尺寸（11.3）
          → ScrollRect 重算滚动范围（11.7.7）
            → 可能再次触发 Layout（11.5 循环依赖）
```

item 数量越多，第三步的遍历成本越高；而这个链条**每次数据变化都要走一遍**。

**所以大规模列表的通用解法是"退出 Layout 体系"**：

| 手段 | 具体做法 | 放弃了什么 |
|------|---------|-----------|
| 手动布局 | 用脚本直接算 `anchoredPosition`，不挂 LayoutGroup | 自动排列、自适应 |
| 固定尺寸 | 预设或缓存 item 高度，不挂 ContentSizeFitter | 内容自适应高度 |
| 节点复用 | 对象池 + 只创建可视区数量的节点 | 实现复杂度上升 |

这三条合起来就是**虚拟列表**：只创建可视区域 + 少量缓冲的节点，滚动时改数据不改结构，Content 高度用 `数据量 × item 高度` 直接算出来。Layout 系统从此完全不参与。

> **完整实现见第 20 章 20.6.4**（含对象池与虚拟列表代码）**和第 24 章 24.2**（10000 条数据的排行榜案例）。本节只交代"为什么必须这么做"——那是 Layout 机制决定的，属于本章范畴；"怎么写"属于性能与实战章节。

配套的两点（细节见第 20 章）：把频繁变化的列表拆到独立 Canvas，避免拖累主界面重建；用固定尺寸容器装动态 Text，防止文本变化引发布局连锁反应。
...
| # | 严重程度 | 章节 | 原文声称 | 实际情况 |
|---|---------|------|---------|---------|
| 1 | 🟡 中等 | 11.1.5 | LayoutRebuilder 是"调度中心"，统一遍历所有待重建节点 | `CanvasUpdateRegistry` 负责统一调度；LayoutRebuilder 只负责单个节点的 Rebuild |
| 2 | 🟡 中等 | 11.3.1 | ContentSizeFitter "读取当前节点上的 Layout 尺寸信息" | ContentSizeFitter 通过 `LayoutUtility` 实时遍历 ILayoutElement 计算尺寸，不是"读取已有信息" |
| 3 | 🟡 中等 | 11.4.6 | "某些 LayoutGroup 会自动强制修改 Anchor" | UGUI 内置 LayoutGroup 和 ContentSizeFitter 都不修改 Anchor，仅通过 `SetChildAlongAxis()` 修改 `anchoredPosition` 和 `sizeDelta` |
