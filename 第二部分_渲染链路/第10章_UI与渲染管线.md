...
## 10.7 URP/HDRP 中的 UI 实践

### 10.7.1 URP 帧流程中的 UI 位置

10.2.1 已给出标准 URP Forward Renderer 的 Pass 顺序。UI Overlay 在 URP 中由 `UniversalRenderPipeline.RenderSingleCamera` 的专用 Pass 处理，对应 RenderGraph 的 DrawOverlay 步骤：行为与 Built-in 一致，在场景所有 Camera（含后处理）之后绘制。

### 10.7.2 ScreenSpaceCamera 模式与后处理

Camera 模式的 UI 参与绑定 Camera 的完整渲染链路，会被该 Camera 的后处理影响：

```
Camera A（depth=0，带 Bloom 后处理）
  └── UI Canvas（Camera 模式，绑定 Camera A） → UI 被 Bloom 影响
Camera B（depth=1，不带后处理）
  └── UI Canvas（Camera 模式，绑定 Camera B） → UI 不受 Bloom 影响
```

### 10.7.3 Render Objects Feature

URP 的 `Render Objects` 按 Layer 过滤，在指定时机额外渲染 UI，常见用途：

- 在 Opaque 和 Transparent 之间渲染 UI，实现"UI 被场景半透明物体遮挡"的效果
- 特定 Camera 的 UI 叠加

注意它可能意外渲染不该渲染的 UI，用 Layer 收窄范围。

### 10.7.4 URP 合批与 SRP Batcher

UGUI 的合批由 `Canvas.BuildBatch()` 独立完成，DrawCall 在进入管线前已经合并。SRP Batcher 优化的是材质属性提交，不改变 UGUI 的 DrawCall 合并结果（详见 10.5）。

### 10.7.5 UGUI 在 HDRP 中
