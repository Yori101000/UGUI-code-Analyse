...
## 10.1 Built-in Pipeline 渲染流程

### 10.1.1 UI 在 Built-in RP 中的渲染位置
...
### 10.1.4 Camera 模式与 World Space 模式
...
> ⚠ **勘误 #3**：10.2.1 的流程图将 Shadow Pass 排在 Depth PrePass 之前，且将 UI 渲染排在 Final Blit 之后。标准 URP Forward Renderer 中 Depth PrePass 在 Shadow Pass 之前，Overlay UI 在 Final Blit 之前。

### 10.2.2 URP 中 UI 的插入位置与扩展性
...
### 10.2.3 三种模式在 URP 中的行为

**Overlay 模式**：在所有 Camera（包括 Camera Stack 中的所有 Camera）完成渲染后才绘制。**不会被任何 Camera 的后处理影响。**

> ⚠ **勘误 #5**：10.2.3 声称切换到 URP 后"某些后处理开始影响 UI"，将其归为 Overlay UI 的行为变化。这是严重错误——后处理影响 UI 只发生在 Camera 模式下，不适用于 Overlay。
...
## 10.3 Camera Stack 与 UI

### 10.3.1 什么是 Camera Stack
...
## 10.4 RenderQueue 与排序

### 10.4.1 什么是 RenderQueue
...
### 10.4.4 Canvas 排序与 RenderQueue 的关系
...
### 10.4.6 UI Shader 与 RenderQueue
...
### 10.5.6 SRP Batcher 对 UI 的真实意义

SRP Batcher 对 UGUI 不是完全无意义，但它是额外优化项而非核心机制。UGUI 属于高动态渲染体系，与 SRP Batcher 擅长优化的"大量稳定 Mesh Renderer"场景不完全一致。

> ⚠ **勘误 #8**：原文第 9 章小结声称"SRP Batcher 使大量 UI 在频繁更新场景下具备更稳定性能表现"，与正文 10.5 节的分析直接矛盾。正文多个子节明确指出 UGUI 几乎无法有效利用 SRP Batcher。正确表述：SRP Batcher 对 UGUI 收益有限，UI 性能核心在于 Canvas Batch 与 Rebuild 控制。

---

## 10.6 UI 在 GPU 渲染阶段的位置
...
### 10.6.3 一条因果链：ZWrite Off → 无法 Early-Z → Overdraw → FillRate

UI 的 GPU 开销几乎全部由这一条链条解释，值得完整串一遍：

```
UI 走透明混合（Blend SrcAlpha OneMinusSrcAlpha）
  → 必须能看到"背后已经画好的颜色"，所以不能写深度（ZWrite Off）
    → GPU 无法用深度缓冲判断"这个像素后面还会被盖住"
      → 拿不到 Early-Z 剔除，被遮挡的片元照样跑完 Fragment Shader
        → 同一像素被反复着色 = Overdraw
          → 每次着色都要"读 FrameBuffer → 混合 → 写回" = FillRate 压力
```

**三个推论**：

1. **UI 的 GPU 成本主要在 Fragment，不在 Vertex。** 一个全屏半透明 Image 即使只有 4 个顶点、纹理再简单，GPU 也得对屏幕每个像素跑一遍片元着色 + 一次读-算-写混合。顶点多一倍不痛不痒，覆盖面积大一倍是实打实的。
2. **"看不见"不等于"没画"。** 被完全遮住的 UI，只要它在渲染顺序里排在前面，片元照样执行。这也是为什么隐藏 UI 要 `SetActive(false)` 或让 `canvasRenderer.cull = true`，光靠盖住没用。
3. **高分屏是乘数。** 像素数翻倍，上面每一层的成本都翻倍——同一套 UI 在 1080p 顺畅、在 2K 掉帧是常态。

**World Space UI 是个例外**：它可以参与 ZTest、被场景物体遮挡。但大多数 UI Shader 仍然 `ZWrite Off`，因为 UI 的设计出发点就是透明混合而非深度写入——所以它只是"能被遮挡"，并没有拿回 Early-Z。

> 减少 Overdraw 只有一条路：**减少实际重叠的像素面积**。全屏半透明遮罩、多层弹窗叠加、大面积 Blur/Glow 是三个主要来源（第 19、20 章）。注意裁剪系统帮不上忙——Mask 和 RectMask2D 都不减少 Overdraw（第 15 章 15.3.3）。
...
## 10.7 URP / HDRP 的额外注意点

前面 10.2（URP 的 Pass 体系）、10.3（Camera Stack）、10.5（SRP Batcher）已经讲完了 SRP 下 UI 的基本行为，这里只补三件在 Built-in 下遇不到的事。

先把已经讲过的三条结论钉住，避免来回翻：

- **UI Overlay 在 URP 中由专用 Pass 处理**（RenderGraph 的 DrawOverlay 步骤），行为与 Built-in 一致——在所有 Camera 含后处理之后绘制（10.2.1）。
- **只有 Camera 模式的 UI 会被后处理影响**，绑哪个 Camera 就吃哪个 Camera 的后处理；Overlay 模式永远不受影响（10.2.3、10.3.6）。
- **SRP Batcher 不改变 UGUI 的 DrawCall 数量**：合批早在 `Canvas.BuildBatch()` 里做完了，SRP Batcher 优化的是材质属性提交（10.5）。

### 10.7.1 Render Objects Feature（URP 特有）

URP 的 `Render Objects` 按 Layer 过滤，在指定时机额外渲染一批对象，用在 UI 上有两个典型场景：

- 在 Opaque 和 Transparent 之间渲染 UI，做出"UI 被场景半透明物体遮挡"的效果
- 给特定 Camera 单独叠加一层 UI

注意它很容易**意外渲染到不该渲染的 UI**（Layer 配宽了就会全部命中），务必用 Layer 收窄范围。

### 10.7.2 UGUI 在 HDRP 中

HDRP 渲染流程更复杂，UI 的插入位置（以实际渲染器配置为准）：
...
HDRP 用 HDCamera 和 Camera Stack 管理多 Camera：Base Camera 渲染场景，Overlay Camera 渲染 UI，UI 与场景叠加并受 Base Camera 的 ToneMapping 影响。
...
| 后处理影响 UI | UI 在 ToneMapping 前渲染 | 用 Overlay 模式或在后处理之后渲染 |
| UI 写入深度 | UI Shader 默认 ZWrite Off | 自定义 UI Shader 按需开启 |
| HDR Color Buffer | HDRP 默认使用 HDR 缓冲区 | UI 在校色前为 HDR，看起来过亮，需在校色后渲染 |
...
- Game UI 用 Overlay 模式，不受后处理影响，性能最好
- CG 过场 UI 用 Camera 模式，与场景后处理风格统一
- UI 要被场景物体遮挡时用 WorldSpace 或 Camera 模式加深度测试
- URP 项目注意 Render Objects Feature 可能意外渲染 UI
- HDRP 项目注意 HDR 缓冲区导致 UI 颜色失真，需在校色后渲染

管线行为随 Unity 版本变化，以上以当前 URP/HDRP 版本为准。
...
从 CPU 侧的批处理构建，到渲染管线的阶段调度，再到 GPU 端的像素输出，每一层机制共同决定了 UI 的最终性能与视觉结果。**UI 不是独立于渲染体系的特殊系统，而是 GPU 透明渲染体系中的一个普通参与者。** 理解这一点，是理解所有 UI 渲染问题（排序、性能、后处理交互）的基础。URP/HDRP 下的行为差异与实战建议见 10.7。
...
```
引擎内置：UI-Default.shader（Data/Resources/BuiltinShaders/）
URP/HDRP：Unity-Technologies/Graphics 仓库（com.unity.render-pipelines.*）
阅读顺序：Canvas.renderMode（第 6 章）→ UI-Default.shader → SRP 包内 Canvas 相关代码
```
...
| 2 | 🔴 严重 | 10.2.3 | Overlay UI 在 URP 中会被后处理影响 | Overlay UI 在所有 Camera Pass 之后渲染，不受后处理影响 |
| 3 | 🔴 严重 | 本章小结 | SRP Batcher 使 UI 具备更稳定性能表现 | 与正文矛盾，UGUI 几乎无法有效利用 SRP Batcher |
| 4 | 🟡 中等 | 10.4.3 | RenderQueue 优先级高于 UI 内部排序规则，能覆盖 Sorting Order | 跨 Canvas 排序由 Sorting Order 主导，RenderQueue 不能覆盖 |
| 5 | 🟡 中等 | 10.1.5 | Sorting Layer、Depth 等被不加区分列为 Overlay UI 排序因素 | Depth 和 Camera Depth 对 Overlay UI 不适用 |
| 6 | 🟡 中等 | 10.2.2 | "UI 可与 Post Processing 协同工作" | 仅 Camera 模式适用，Overlay 不适用 |
| 7 | 🟢 轻微 | 10.2.1 | URP 流程：Shadow → Depth PrePass → … → Final Blit → UI | Depth PrePass 在 Shadow 前；UI 在 Final Blit 前 |
| 8 | 🟢 轻微 | 10.5.5 | "使用 MaterialForRendering"导致材质实例化 | materialForRendering 是只读 getter，访问不创建实例 |
| 9 | 🟢 轻微 | 10.6.2 | Overlay UI 顶点经过"MVP 矩阵变换" | Overlay 是屏幕空间坐标，使用正交投影，不经过传统 MVP |
