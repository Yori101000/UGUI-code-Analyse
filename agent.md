...
2. **勘误表已改为如实说明**：原先 07/13/20/21/22/23/24/25 章的占位行「本章无源码级勘误记录」会被误读为"已核查"，现已改为列出「已核查项 / 未核查项」，或明确说明该章不含源码事实声明。新增勘误请继续沿用「原文声称 / 实际情况」对照格式。
   **第 13 章已于 2026-08-14 补做逐条核查**（对照 main `Selectable.cs` / `Button.cs` / `ColorBlock.cs`），查出 7 项，其中 2 项 🔴：`SelectionState` 实为五个成员（原文写四个并断言 Selected 与 Highlighted 无区别）、状态判定在 `currentSelectionState` 属性且 `hasSelection` 命中的是 `Selected`。目前**仅剩 21/22 章无勘误条目，但这两章本就不含源码事实声明**。
3. **历史 / 过期文档，勿当现状引用**：
