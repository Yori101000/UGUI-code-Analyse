...
- **F6 颗粒度失衡**（未做）：第 19 章 36.4KB / 第 10 章 33.4KB / 第 11 章 32.8KB，而第 25 章仅 6.7KB。优先按"效果/维度"做章内重排或拆分段落——这类改动不影响交叉引用，可直接做。**若判断必须拆成新文件、需要重新编号，先向用户说明理由并征求意见，得到确认后再动手**（代价是全库交叉引用与 `README.md` 目录/依赖图都要跟着改，见第 5 节）。
