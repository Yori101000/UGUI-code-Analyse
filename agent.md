...
- **F5 收尾体例统一**（未做）：全库存在 `## 本章总结` / `## N.x 本章总结` / `## 总结` 三种混用；「源码阅读路径」段有的在勘误汇总之前、有的在之后（21/24/25 章在后）。建议统一为 `## 本章总结` → `## 源码阅读路径` → `## 勘误汇总`，并把该顺序写进本文件第 3 节的章节模板。属全库性改动，应独立提交。
- **F6 颗粒度失衡**（未做）：第 19 章 33.5KB / 第 10 章 33.4KB / 第 11 章 32.8KB，而第 25 章仅 5.7KB。如需精简，按"效果/维度"做章内重排或拆分段落即可，**不建议拆文件重编号**（避免全库交叉引用失效）。
