...
> 🔴 **最高优先级：全库审查报告（2026-08-14）的 47 项问题尚未修复**，清单见 `_阅读指南/全库审查报告（2026-08-14）.md`。**回答问题或修改内容前应先查该报告**，避免引用已知错误的表述。重点：
>
> - **P0 跨章矛盾 12 项**：RectMask2D 的机制与断批（08/09/14/15/20/24 六章四种互斥说法）、`Graphic.material` 语义（20.2 整节立论错误）、`DoMeshGeneration` / `UpdateMaterial` / `FillMesh` 的新旧写法并存（04/18 章未随 02/05/08 同步）、Stencil 与 Fragment Shader 时序（15/20 vs 18）、Mask 内子元素能否合批（15 vs 24）等。
> - **P0 编造 API 5 项**：04.6 两张表的方法名（`Graphic.SetColor()` / `Image.SetSprite()` / `Text.SetText()` 等）在 uGUI 中均不存在；20.2 的 `image.sharedMaterial` 在 `Graphic` 上不存在，示例编译不过。
> - **P1/P2**：18/19 章 Shader 示例缺 `#include "UnityCG.cginc"`、无测量条件的性能数字（已定改为定性描述）、收尾体例不统一等。

1. **颗粒度不均**：第 16、19、20 章在 30KB 量级，第 07、21 章等偏小；合并产生的章节内部结构仍在演进。
2. **模板段流于形式**：全部 25 章虽都有「源码阅读路径 / 勘误汇总」段，但 07/13/20/21/22/23/24/25 章的勘误表是占位行「本章无源码级勘误记录」，其中 **20 章那条已被审查报告 A2/B2 证伪**。空表会被误读为"已核查"，见报告 E1。
3. **历史 / 过期文档，勿当现状引用**：
   - `_阅读指南/整体结构分析（历史存档）.md` —— 早期重构规划，已明确标注存档。
   - `_阅读指南/第1-8章源码勘误.md` —— ⚠ **过期且反向错误**：以 2019.4/2020.3 为基线（违反 main 基线强制条款），并把 02 章已标 🔴 的「UIVertex 定义在 VertexHelper.cs」认证为「✅ 一致」，把已有 4 条勘误的 08 章认证为"无错误"。详见报告 E2。**该文件本身尚未加过期标注。**
   - `_阅读指南/源码一致性审计报告.md` —— 结论仍有效，但其"范围：全部 25 章"的声明不实，实际只覆盖 01/03/04/06/08/09/16/17/19 九章，见报告 E3。
...
- ⚠ **以下话题不要直接引用正文现有表述，先查 `_阅读指南/全库审查报告（2026-08-14）.md` 的统一口径**：
  - **RectMask2D 的机制与是否断批**（报告 A1）——正文六章四种说法互斥
  - **`Graphic.material` / `sharedMaterial` 的语义**（报告 A2、B2）——20.2 整节错误
  - **Stencil 测试与 Fragment Shader 的先后**（报告 A6）——以 18.4.3 的条件表述为准
  - **Mask 内子元素能否合批**（报告 A7）——以 15 章为准，24.5.3 案例前提错误
  - **哪些 API 会触发 Dirty**（报告 B1）——04.6 两张表的方法名均为编造，实际是属性 setter
...
- **优先做审查报告的 P0 批次**（A+B+E，共 22 条），建议顺序：E2（给过期文档加标注，成本最低）→ A1（RectMask2D 统一口径，涉及面最广）→ A2/B2（20.2 整节重写）→ B1（04.6 两张表）→ 其余。详见报告末尾「修复优先级建议」。
- 第 19/20 章体量仍偏大：如需进一步精简，按"效果/维度"做内部重排或拆分段落即可，不建议拆文件重编号（避免全库交叉引用失效）。
- 内容增补方向（报告 G 节）：25 章过薄可补 UI Toolkit 渲染路径与共存细节；`CanvasGroup` 缺专门讲解；`Image` 的 Sliced/Tiled/Filled 与 Simple 详略不对称；`Dropdown`/`InputField` 仅在 13 章列名。
